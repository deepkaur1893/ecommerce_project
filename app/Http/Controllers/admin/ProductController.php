...
class ProductController extends Controller
{
    ...
    public function manage_product_process(Request $request)
    {
        ...
        $request->validate([                        // do validation
            ...
            'arr_image.*'=>'mimes:jpeg,jpg,png',      /// validation for array variable by using *
            'images.*'=>'mimes:jpeg,jpg,png'      /// validation for array variable by using *
        ]);

        /// validation from backend data
        ...
        foreach($skuArr as $key=>$val)
        {
            ...
            if(isset($check[0]))
            {
                ...
                return redirect(request()->headers->get('referer'));                ////redirect on back page
            }
        }
        // end of validation
        if($request->post('id')>0)
        ...
        if($request->hasfile('image'))
        {
            if($request->post('id')>0)
            {
                ...
                {
                    Storage::delete('/public/media/'.$arrImage[0]->image);   /////delete image from folder
                }
            }
        ...
        foreach($skuArr as $key=>$val)
        {
            ...
            if($request->hasFile("attr_image.$key"))
            {
                if($paidArr[$key]>0)
                {
                    ...
                    {
                        Storage::delete('/public/media/'.$arrImage[0]->attr_image);   /////delete image from folder
                    }
                }
        ...
        foreach($piidArr as $key=>$val)
        {
            ...
            if($request->hasFile("images.$key"))
            {
                if($piidArr[$key]>0)
                {
                    ...
                    {
                        Storage::delete('/public/media/'.$arrImage[0]->images);   /////delete image from folder
                    }
                }
                $rand=rand('111111111','999999999');
